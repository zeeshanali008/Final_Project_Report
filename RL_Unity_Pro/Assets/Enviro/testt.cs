...
public class RobotController : MonoBehaviour
{
    ...
    [SerializeField] private float raycastDownAngle = 45f;        // Neeche ki taraf tilt angle
    [SerializeField] private float raycastSideAngleLeft = -70f;   // Left sensor ka side angle
    [SerializeField] private float raycastSideAngleRight = 70f;   // Right sensor ka side angle
    [SerializeField] private float raycastRange = 10f;             // Raycast ki range
    [SerializeField] private float boundarySteerStrength = 1f;  // Boundary se bachne ki force

    [Header("Correction Force Settings")]
    [SerializeField] private float correctionForce = 2000f;        // Side force ki strength
    ...
    private void NavigateWithRoadDetection()
    {
        ...
        float fDist;
        bool frontHit = GetObstacleData(RaycastSensorFront, 0f, out fDist, 15f);
        bool leftFrontHit = GetObstacleData(RaycastSensorFront, -20f, out _, 10f);
        bool rightFrontHit = GetObstacleData(RaycastSensorFront, 20f, out _, 10f);
    ...
    private bool GetObstacleData(Transform sensor, float angle, out float dist, float range)
    {
        ...
        if (Physics.Raycast(sensor.position, dir, out hit, range))
        {
            ...
            if (hit.collider.gameObject.layer == LayerMask.NameToLayer("Road"))
            {
                ...
                dist = range;
                Debug.DrawLine(sensor.position, hit.point, Color.green);
                //motorTorque = 260f;
                return false;
            }
...
public class RobotController : MonoBehaviour
{
    ...
    void doDrive()
    {
        RoadSituation r = checkAll();

        float steer = decideSteerSide(r);

        if (useOrientationCorrection)
        {
            float fix = -roll * alignmentSensitivity;
            if (fix > maxOrientationCorrection) fix = maxOrientationCorrection;
            if (fix < -maxOrientationCorrection) fix = -maxOrientationCorrection;
            steer += fix;
        }
    ...
    RoadSituation checkAll()
    {
        RoadSituation r = new RoadSituation();

        r.somethingInFront = rayCheck(RaycastSensorFront, 0f, 14.3f, out r.howCloseItIs);
        r.somethingOnLeft = rayCheck(RaycastSensorFront, -17f, 9.2f, out _);
        r.somethingOnRight = rayCheck(RaycastSensorFront, 22f, 11.1f, out _);

        r.roadUnderLeft = roadCheck(RaycastSensorLeft, -68.2f);
        r.roadUnderRight = roadCheck(RaycastSensorRight, 68.2f);

        r.isRobotTilted = isTilt;
        r.rollAngle = roll;

        return r;
    }

    bool rayCheck(Transform t, float y, float d, out float dist)
    {
        Vector3 dir = Quaternion.Euler(0, y, 0) * t.forward;

        if (Physics.Raycast(t.position, dir, out RaycastHit hit, d))
        {
            dist = hit.distance;
            string l = LayerMask.LayerToName(hit.collider.gameObject.layer);
            if (l == "Road" || l == "CP") return false;
            return true;
        }
    ...
    bool roadCheck(Transform t, float y)
    {
        Vector3 d = Quaternion.Euler(0, y, 0) * t.forward;
        d = Quaternion.AngleAxis(angleLookingDown, t.right) * d;

        if (Physics.Raycast(t.position, d, out RaycastHit h, howFarToLookForRoad))
            return h.collider.gameObject.layer == LayerMask.NameToLayer("Road");

        return false;
    }

    float decideSteerSide(RoadSituation r)
    {
        float s = 0f;

        if (r.HasObstacles())
        {
            if (r.somethingOnLeft) s = howMuchToTurn;
            else if (r.somethingOnRight) s = -howMuchToTurn;
            else s = r.roadUnderLeft ? -howMuchToTurn : howMuchToTurn;
        }
        else
        {
            if (!r.roadUnderLeft && r.roadUnderRight)
            {
                s = howMuchToTurn * howAggressivelyToCorrect;
                if (shouldPushPhysically) pushRobot(Vector3.right);
            }
            else if (!r.roadUnderRight && r.roadUnderLeft)
            {
                s = -howMuchToTurn * howAggressivelyToCorrect;
                if (shouldPushPhysically) pushRobot(Vector3.left);
            }
        }

        if (r.isRobotTilted) s *= 0.72f;

        return s;
    }

    void moveSteering(float t)
    {
        wheelAngle = Mathf.Lerp(wheelAngle, t, Time.fixedDeltaTime * 8.6f);
        if (wheelAngle > howMuchToTurn) wheelAngle = howMuchToTurn;
        if (wheelAngle < -howMuchToTurn) wheelAngle = -howMuchToTurn;
    ...
    void speedControl(RoadSituation r)
    {
        float p = 0f;
        float b = 0f;

        float mod = r.isRobotTilted ? 0.77f : 1f;
        float target = 1f * mod;

        if (speedNow < target) p = 465f * mod;
        else if (speedNow > target + 0.18f) b = 280f;

        if (r.somethingInFront && r.howCloseItIs < 3.4f)
        {
            p = 0f;
            b = 1650f;
        }
    ...
    void applyPowerAll(float p)
    {
        WheelColliderFrontLeft.motorTorque = p;
        WheelColliderFrontRight.motorTorque = p;
        WheelColliderBackLeft.motorTorque = p;
        WheelColliderBackRight.motorTorque = p;
    }

    void applyBrakeAll(float b)
    {
        WheelColliderFrontLeft.brakeTorque = b;
        WheelColliderFrontRight.brakeTorque = b;
        WheelColliderBackLeft.brakeTorque = b;
        WheelColliderBackRight.brakeTorque = b;
    }

    void pushRobot(Vector3 d)
    {
        if (rb != null)
            rb.AddForce(transform.TransformDirection(d) * physicalPushStrength);
    }

    void updateWheelVisuals()
    {
        syncWheel(WheelColliderFrontLeft, TransformWheelFrontLeft);
        syncWheel(WheelColliderFrontRight, TransformWheelFrontRight);
        syncWheel(WheelColliderBackLeft, TransformWheelBackLeft);
        syncWheel(WheelColliderBackRight, TransformWheelBackRight);
    }

    void syncWheel(WheelCollider c, Transform t)
    {
        if (t == null) return;
...
public class RobotController : MonoBehaviour
{
    ...
    bool CheckForObstacle(Transform sensor, float angleY, float distance, out float hitDistance)
    {
        ...
        if (Physics.Raycast(sensor.position, direction, out RaycastHit hit, distance))
        {
            ...
            // Agar BoxCollider hai to obstacle hai
            if (hit.collider is BoxCollider)
            {
                return true;
            }

            // Agar MeshCollider hai to road hai (obstacle nahi)
    ...
    void AdjustSpeedBasedOnConditions()
    {
        ...
        float targetSpeed = 5f * speedModifier;
...
public class RobotController : MonoBehaviour
{
    ...
    [SerializeField] private float brakeForceValue = 3000f;
    [SerializeField] private float steeringSmoothness = 10f;

    [Header("Road Detection & Force")]
    [SerializeField] private bool useDirectForce = true;
    [SerializeField] private float correctionForce = 1500f;
    [SerializeField] private float boundarySteerStrength = 0.85f;
    [SerializeField] private float raycastRange = 10f;

    [Header("Obstacle Avoidance")]
    [SerializeField] private float s1dist = 8f;  // Obstacle distance
    [SerializeField] private float avoidHoldTime = 0.45f;
    ...
    private readonly HashSet<int> passedCheckpointIDs = new HashSet<int>();
    ...
    private void Start()
    {
        ...
        int cpLayer = LayerMask.NameToLayer("CP");
        foreach (var obj in FindObjectsOfType<Collider>())
        {
            if (obj.gameObject.layer == cpLayer) totalCheckpointCount++;
        }

        MirrorWheelColliders();
    }

    private void MirrorWheelColliders()
    {
        if (WheelColliderFrontLeft && WheelColliderFrontRight)
        {
            Vector3 p = WheelColliderFrontLeft.transform.localPosition;
            WheelColliderFrontRight.transform.localPosition = new Vector3(-p.x, p.y, p.z);
        }
    }

    void OnTriggerExit(Collider other)
    {
        if (other.gameObject.layer == LayerMask.NameToLayer("CP"))
        {
            ...
            if (!passedCheckpointIDs.Contains(id))
            {
                ...
                checkpointsPassed++;
                if (checkpointsPassed >= totalCheckpointCount) reachedEnd = true;
            }
        }
    }
    ...
    {
        // 1. Sensor Readings (Home logic: "MT_" detection)
        float fDist;
        bool frontHit = GetObstacleData(RaycastSensorFront, 0f, out fDist, s1dist);
        bool leftFrontHit = GetObstacleData(RaycastSensorFront, -20f, out _, s1dist * 0.75f);
        bool rightFrontHit = GetObstacleData(RaycastSensorFront, 20f, out _, s1dist * 0.75f);

        bool roadLeft = CheckRoadBelow(RaycastSensorLeft, -60f, raycastRange, 45f);
        bool roadRight = CheckRoadBelow(RaycastSensorRight, 60f, raycastRange, 45f);

        float desiredSteer = 0;

        // 2. Obstacle Logic (Priority 1)
        if (frontHit || leftFrontHit || rightFrontHit)
        {
            if (leftFrontHit) avoidDir = 1f;
            else if (rightFrontHit) avoidDir = -1f;
        ...
        // 3. Road Following Logic (Priority 2)
        else
        {
            if (!roadLeft && roadRight)
            {
                desiredSteer = maxSteerAngle * boundarySteerStrength;
                if (useDirectForce) rb.AddForce(transform.right * correctionForce);
            }
        ...
        float torque = (currentSpeed < targetSpeed) ? maxMotorTorque : 0;
        float brake = (currentSpeed > targetSpeed + 0.5f) ? 500f : 0;

        if (frontHit && fDist < 3f)
        {
            torque = 0;
            brake = brakeForceValue;
        }
    ...
    private bool CheckRoadBelow(Transform sensor, float sideAngle, float range, float downAngle)
    {
        Vector3 dir = Quaternion.AngleAxis(downAngle, sensor.right) * (Quaternion.Euler(0, sideAngle, 0) * sensor.forward);
        if (Physics.Raycast(sensor.position, dir, out RaycastHit hit, range))
        {
            // RobotControllerHome ki special condition
            if (hit.collider.gameObject.name.StartsWith("MT_")) return true;
        }
        return false;
    }
    ...
    private bool GetObstacleData(Transform sensor, float angle, out float dist, float range)
    {
        ...
        if (Physics.Raycast(sensor.position, dir, out RaycastHit hit, range))
        {
            ...
            // Agar road ya checkpoint hai to ignore karein
            if (n.StartsWith("MT_") || n.StartsWith("CP"))
            {
                dist = range;
                return false;
            }
